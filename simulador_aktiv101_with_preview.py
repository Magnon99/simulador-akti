...
def calcular(e: Entradas) -> Saidas:
    ...
    rs_flags_bruto = {
        k: kwh_flags[k] * (getattr(e, k.lower().replace(" ", "_")).tarifa_rs_kwh if getattr(e, k.lower().replace(" ", "_")) and getattr(e, k.lower().replace(" ", "_")).ativo else 0)
        for k in kwh_flags
    }
    ...
    if e.bonus_itaipu_ativo: custo_sem -= e.bonus_itaipu_rs
    if e.comp_indic_ativo:   custo_sem -= e.comp_indic_rs
...
def exportar_png(titulo_mes: str, titulo_ano: int, e: Entradas, out: Saidas) -> bytes:
    ...
    if e.comp_indic_ativo and e.comp_indic_rs: linhas.append({"label":"Compensação por indicador","valor":-abs(e.comp_indic_rs),"calc":None})
    if e.bonus_itaipu_ativo and e.bonus_itaipu_rs: linhas.append({"label":"Bônus Itaipu","valor":-abs(e.bonus_itaipu_rs),"calc":None})

    header_h=24+STEP*len(linhas)+60; H=max(620,header_h+360)
    img=Image.new("RGB",(W,H),(255,255,255)); d=ImageDraw.Draw(img)
    d.rectangle([0,0,W,header_h],fill=BG_TOP)

    x_label=22; x_amount=x_label+d.textlength(consumo_label,font=f28); y=16
    for idx,row in enumerate(linhas):
        ...
        if row["calc"]:
            calc=row["calc"]; calc_x=x_amount+d.textlength(fmt_rs(row["valor"]),font=f28)+22
...
desconto_pct=st.number_input("Percentual de desconto (%)",0.0,100.0,20.0,0.5,format="%.1f")
st.divider()
...
with col1:
    ...
    consumo_kwh=st.number_input("Consumo do período (kWh):",0.0,step=1.0,value=0.0,format="%.2f")
    valor_consumo=st.number_input("Valor do consumo (R$):",0.0,step=0.01,value=0.0,format="%.2f")
    ip_reais=st.number_input("Valor da Iluminação Pública (R$):",0.0,step=0.01,value=0.0,format="%.2f")

    st.markdown("#### Energia injetada (GDII / ATVs)")
    n_gdii=st.number_input("Quantas GDII (ATVs) existem?",0,12,1)
    atvs=[]
    if n_gdii>0:
        with st.expander("Preencher cada GDII / ATV",expanded=True):
            for i in range(int(n_gdii)):
                c1,c2=st.columns(2)
                kwh=c1.number_input(f"GDII #{i+1} – kWh",0.0,step=0.01,value=0.0,format="%.2f",key=f"gdii_{i}_kwh")
                rs=c2.number_input(f"GDII #{i+1} – Valor (R$)",0.0,step=0.01,value=0.0,format="%.2f",key=f"gdii_{i}_rs")
    ...
    comp_rs=st.number_input("Compensação por Indicador (R$)",0.0,step=0.01,value=0.0,format="%.2f",disabled=not comp_on)
    bonus_on=st.checkbox("Tem **Bônus Itaipu**?",value=True)
    bonus_rs=st.number_input("Bônus Itaipu (R$)",0.0,step=0.01,value=0.0,format="%.2f",disabled=not bonus_on)
    ...
    def ui_bandeira(label,default_mes,def_tarifa,def_dias_ban,keyprefix):
        ...
        tarifa=colb2.number_input(f"Tarifa {label} (R$/kWh)",0.0,step=0.00001,value=def_tarifa,format="%.5f",key=f"{keyprefix}_tar")
        d_leit=colb3.number_input(f"Dias da leitura ({label})",1,step=1,value=30,key=f"{keyprefix}_dl",disabled=not ativo)
        d_ban=colb4.number_input(f"Dias com bandeira ({label})",0,step=1,value=def_dias_ban,key=f"{keyprefix}_db",disabled=not ativo)
...
with col2:
    ...
    ano_png=c2.number_input("Ano",2000,2100,2025,step=1)

    e=Entradas(consumo_kwh=consumo_kwh,valor_consumo_reais=valor_consumo,
               iluminacao_publica_reais=ip_reais,atvs=atvs,
               ...
               vermelha_p1=vermelha_p1,vermelha_p2=vermelha_p2,
